...
3. Enable Bugbot on:
   - `https://github.com/dolab-jpg/sync2dine-frontend`
   - `https://github.com/dolab-jpg/sync2dine-backend`
...
- Supabase migrations live only in `sync2dine-backend/supabase/migrations/`.
...
1. Set on the **live server** (and optionally local `.cursor/local/deploy.env`):
   ...
   - `GITHUB_TOKEN` — GitHub PAT with `repo` scope on `dolab-jpg/sync2dine-frontend` and `sync2dine-backend` (needed for **Approve & merge** from chat / AI Audit)
2. In Cursor Dashboard, grant the API key’s GitHub App access to both repos.
3. Confirm green **LIVE** on `/ai-audit` → **Code fixes**, and the LIVE badge in the Cynthia AI chat header.
...
| Area | Repo |
|------|------|
| Frontend (Vite/React `src/`) | sync2dine-frontend |
| Backend (API + Supabase) | sync2dine-backend |

Live product: **https://app.sync2dine.io**. Deploy: `bash scripts/push-live-local.sh`.
