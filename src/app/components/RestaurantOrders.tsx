...
function OrderCard({
  ...
  const tier = slaTier(order);
  const itemsWithAllergens = preview.map((item) => {
    if (item.allergensContains?.length) return item;
    const key = (item.name || item.label).toLowerCase();
    const fromMenu = menuAllergens.get(key);
    return fromMenu?.length ? { ...item, allergensContains: fromMenu } : item;
  });
  ...
  return (
    ...
    >
      <header className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 pb-3">
        <div>
          ...
          <p className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-sm font-semibold text-slate-600">
            ...
            <CallRecordingBadge recordingUrl={order.recordingUrl} />
            {order.phone ? (
              <a
      ...
      <div className="mt-3 space-y-2">
        {order.customerAllergies ? <AllergyBanner text={order.customerAllergies} /> : null}
        <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
          ...
          <div className="rounded-2xl bg-slate-950 p-4 text-center text-white">
            <p className="text-3xl font-black">£{order.total.toFixed(2)}</p>
          </div>
        </div>
      </div>

      <footer className="mt-4 space-y-2" onClick={(e) => e.stopPropagation()}>
        {bump && (
          <Button
            type="button"
            data-testid={`order-bump-${order.id}`}
            className="min-h-12 w-full rounded-xl bg-s2d-gold text-base font-bold text-s2d-teal-deep hover:bg-s2d-gold-soft"
        ...
        <div className="flex gap-2">
          ...
          >
            {moreOpen ? <ChevronDown className="mr-1 h-4 w-4" /> : <MoreHorizontal className="mr-1 h-4 w-4" />}
            More
          </Button>
          <CallRecordingPlayer recordingUrl={order.recordingUrl} compact className="flex-1" testId={`order-recording-${order.id}`} />
        </div>
        {moreOpen && (
          <SecondaryActions order={order} patchOrder={patchOrder} onOpenMore={() => openOrder(order)} />
        )}
      </footer>
    </article>
  );
