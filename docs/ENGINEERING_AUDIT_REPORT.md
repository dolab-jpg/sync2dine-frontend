...
## 2. Overall Repository Health � **56 / 100**
...
| Deploy path clarity | +8 | `scripts/push-live-local.sh`, VPS dir `�/sync2dine.io/sync2dine-backend` |
...
| Legacy / dual-product residue | ?6 | FE branding, `auto-ssl-app.sh`, vite �tradepro-backend� comments |
| Bundle / FE hygiene | ?4 | ~2.8 MB main chunk; unguarded `:7756` ingest calls |

**56** = shippable product surface with serious structural debt. Not a greenfield �clean architecture� score.
...
1. **Root re-export stubs** keep flat import paths (`./vapi-routes`, `./phone-webhook`) while canonical code lives in domain folders � intentional, but agents still edit the wrong file if maps fail.
...
4. **Deploy dual path**: local `push-live-local.sh` (excludes `.env`, `server/data`) vs GitHub Actions SCP of `.` (exclusion behaviour for secrets/data **Unverified** � `appleboy/scp-action` source `.` without documented exclude).
...
**Conclusion:** Shared Sally BI docs (`SALLY_ARCHITECTURE.md`) match the *intended* split. Execution modules after splits are **not internally consistent**. Happy-path web Q&A can work without hitting broken tool branches; that must not be read as �AI layer healthy.�
...
Workers started from `server/index.ts` listen callback: mailbox poller, outbound worker, connector queue, sales-brain, Sally KB + cache warm, scheduled messages, weekly billing, code-fix, WhatsApp Web client. Persistence mix: Supabase when configured, else JSON/memory � **Unverified** which store is live for each worker on VPS without env inspection.
...
- `npm run build` succeeds; `test:bridge` passes (63 tests observed in prior session � **re-run Unverified here**).

**Weaknesses**

- Dev proxy default `http://127.0.0.1:3001` with comments still naming **tradepro-backend** (`vite.ai-plugin.ts`) � works only if local BE uses 3001; live is 3011.
...
- Playwright suite named `test:responsive` is broad; many flows need seeded creds � not a production gate.
...
- Quarantine still typechecked ? noise + false confidence that �legacy is inert.�
...
**Auth pattern:** per-handler, not framework middleware. JWT secret falls back to `'tradepro-dev-jwt-secret-change-in-production'` when `JWT_SECRET` unset (`server/auth.ts`). `AUTH_ENFORCED` appears in a **small** set of files (`auth.ts`, mailbox, calendar, connectors) � docs that imply blanket `/api/ai/*` auth are **drifted**.
...
**Living / mostly aligned:** `AGENTS.md` (both), `docs/ARCHITECTURE.md`, `DEPLOYMENT_MAP.md`, `PHONE_ARCHITECTURE.md`, `SALLY_ARCHITECTURE.md`, ADRs 001�007, domain READMEs, `LEGACY_ALIASES.md`.
...
| Capability inventories marking �LIVE� | Often mean �wired in code,� not �typecheck-clean / toolpath-proven� |
...
| Archive Builder Diddies mermaid | Correctly historical � leave archived |
| Backend prior to audit | No Mermaid � gap closed with verified diagrams |
...
1. `server/ai/orchestrator/handle.ts` � re-bind helper imports  
2. `server/sally/execute.ts` � restore draft/terms helpers or import from offer/session module  
3. `server/sally/tools.ts` � import `SALLY_TOOL_NAMES`  
4. `server/phone/tools/execute.ts` � fix `sally-receptionist` path; import `isStaffPartyPhone`  
5. `server/supabase-data.ts` � type/contract drift (28 errors)  
6. `server/whatsapp-webhook.ts` � unbound `from` / `phoneNumberId` / `accessToken`  
7. `server/data-store.ts` � replace global `requestOrgId`  
8. `server/auth.ts` � remove JWT fallback secret  
9. `server/_quarantine/**` � exclude from `tsc`  
10. FE `:7756` ingest call sites (8 files)  
11. `scripts/auto-ssl-app.sh` � delete or hard-disable  
12. `vite.ai-plugin.ts` comments / default port documentation  
13. `analytics-routes.ts` � mount or delete  
...
|------|----------|---------|
| Offer facts | Phone imports shared `formatOfferFactsBlock` from `sally/offer.ts` | Shared after Phase 4 � good |
| SaaS packages | FE `engine/saas/saasPackages.ts` mirrors BE `saas-packages` | Intentional mirror � drift risk |
| Planning action names | FE `planningActionNames.ts` ? BE planning tools | Mirror � needs sync test |
...
| Item | Still live? |
|------|-------------|
| `ensureBdiddiesHomeOrg` boot | Yes � `index.ts` |
| JWT secret name `tradepro-dev-�` | Yes � fallback |
| Cynthia construction phone brain | Yes � when purpose/persona matches |
| Cyrus routes / widgets | Yes |
| FE `tradepro_*` storage/events | Yes |
| Building-control / projects / recruitment routes | Yes � construction surface |
| `auto-ssl-app.sh` | Yes � wrong product host |
...
3. **Org spoofing risk** if handlers trust `X-Org-Id` / body `orgId` without enforced auth � pattern present; enforcement uneven.  
4. **Module-global org context** can cross-contaminate concurrent requests (`data-store.ts`).  
5. **Sally Web** is intentionally public � rate limit/CORS must remain correct; secrets must never land in widget responses (logging discipline exists in web path � depth **Unverified**).  
6. **GitHub SCP of repo root** may copy local artifacts if present on runner checkout (usually clean) � still weaker than rsync excludes.  
7. FE **local ingest** endpoints can leak operational metadata to any local listener.  
8. Stripe/Vapi signature verification � code present; live secret correctness **Unverified**.

---

## 17. Performance Observations

- FE main JS ~2.8 MB minified / ~838 KB gzip � slow mobile first paint risk.  
- Many workers use `setInterval` polling � acceptable at small scale; watch mailbox/WhatsApp/Puppeteer memory on VPS.  
- Orchestrator tool rounds + vision path are heavy when fixed � no evidence of request-level concurrency limits beyond Sally Web rate limit.  
...
| Fixing phone tool imports | Receptionist/staff callback paths start working � may expose latent logic bugs |
...
| Removing construction experience | High product risk � still mounted |
| Changing deploy CI excludes | Could delete VPS `.env` if misconfigured � test carefully |
...
## Executive Conclusion � Top 10 improvements (impact � effort)

1. **Re-bind split imports** in `orchestrator/handle.ts`, `sally/execute.ts`, `sally/tools.ts`, `phone/tools/execute.ts` � highest latent outage risk, localized fixes.  
2. **Exclude `_quarantine` from `tsc` and fail CI on typecheck** � stops silent structural rot.  
3. **Delete or hard-fail `auto-ssl-app.sh`; strip `:7756` ingest** � cheap safety.  
4. **Remove JWT fallback; fail boot without `JWT_SECRET` in production** � small change, large security win.  
5. **Replace global `requestOrgId` with AsyncLocalStorage / explicit args** � correctness under load.  
6. **Document + enforce single deploy contract** (local script excludes mirrored in CI) � prevents data/env accidents.  
7. **Auth audit of high-value `/api/*` families** vs FE header trust � security.  
8. **Decide dual-product fate** (extract construction vs quarantine UI) � largest maintainability lever.  
9. **FE code-split / drop unused construction weight for restaurant tenants** � performance.  
10. **Mirror sync tests** for SaaS packages + tool names FE?BE � prevents silent commercial drift.
...
| Encryption/JWT coupling incident | Rotating `JWT_SECRET` briefly broke org OpenAI decrypt (`Unsupported state or unable to authenticate data`) because `crypto.ts` fell back to `JWT_SECRET`. Restored via `ORG_ENCRYPTION_KEY` = legacy encryption material; **decoupled** crypto from JWT. Live Sally Web **200** after fix. |
...
- **Final login / permissions / production lockdown** � see BE [`LIVE_TESTING_ACCESS.md`](../../sync2dine-backend/docs/LIVE_TESTING_ACCESS.md) (recommendation only; no gates during live testing)
