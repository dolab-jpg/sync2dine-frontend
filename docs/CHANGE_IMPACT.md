...
| Orders / menu / deals | Judie phone, restaurant UI, connectors | Judie, staff UI | `PHONE_TOOLS` food/reservations; `orders/*` | order-service, food-guards, meal-deals docs | `/api/orders`, `/api/menu` | CAPABILITY ù24.J, MEAL_DEALS | API + SPA |
...
| Inbox phone thread playback | `/whatsapp` | staff | `CyrusConversations` + `/api/calls` | manual | `/api/calls`, `/api/ai/summarize` | CHANGE_IMPACT | API + SPA |
...
| Billing / Stripe | checkout, webhooks | ù | stripe routes + gap money tools | quote-checkout, weekly billing | `/api/stripe` | CAPABILITY, ROUTE_MAP | API |
| DID / phone lines / org | all phone | Judie/Sally | phone-lines resolve | did-routing.test | vapi + agent lines | PHONE, CHANGE_IMPACT | API |
| Self-heal repos | staff | Cynthia tooling | code-fix-handler | ù | `/api/ai/code-fix` | must stay sync2dine-* remotes | API |
| Auth / org context | all | ù | auth.ts, account-auth | ù | `/api/auth` | CAPABILITY | API |
| Deploy scripts | ù | ù | push-live-local | health curls | ù | DEPLOYMENT_MAP | ù |

## Dual-implementation traps

| If you editù | Also checkù |
|--------------|-------------|
| `phone/sally-sales-phone.ts` tools | `sally/tools.ts` full pack ó do not assume Vapi has web tools |
