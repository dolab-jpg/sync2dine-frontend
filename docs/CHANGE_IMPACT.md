...
| Sally outbound queue policy | `/calls` + `/crm` + worker | Sally | `queue-crm` `allCrm`, no global quiet skip, stale `dialling` reclaim, Call Centre knobs removed | live queue + Call Register | `/api/campaigns/queue-crm`, `/api/agent/settings`, outbound worker | WORKERS, SALLY, APPLICATION_MASTER §24.F | API + SPA |
