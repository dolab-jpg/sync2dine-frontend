...
| CSV campaign research + Cynthia progress | Vapi Sally + Cynthia chat | Sally, Cynthia | `outbound-campaigns` WithResearch, `campaign-progress`, `getCampaignProgress`, outbound voice health | `sally/campaign-ops.test.ts` | `/api/campaigns/upload`, `/api/campaigns/progress` | TOOL_REGISTRY, ROUTE_MAP, CAPABILITY | API + SPA |
