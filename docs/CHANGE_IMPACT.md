...
| Sally gatekeeper / manager handoff | Vapi Sally outbound | Sally sales | opener matrix, `captureLead`/`captureReferralAndQueue` optional names, `gatekeeper_manager_callback` → `called`, silence/endpointing, Sally assistant-request fallback | `sally-gatekeeper.test.ts`, tier1 | vapi + `/crm` disposition labels | SALLY, PHONE, TOOL_REGISTRY, CAPABILITY | API + SPA |
