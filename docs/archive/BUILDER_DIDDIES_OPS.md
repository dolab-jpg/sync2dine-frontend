# Builder Diddies / tradepro ops inventory (HISTORICAL)

> **HISTORICAL — do not deploy, verify, or edit against this document.**
>
> Live Sync2Dine SoT:
> - App/API: **https://app.sync2dine.io**
> - Deploy: `bash scripts/push-live-local.sh` (frontend repo)
> - Agent maps: [../APPLICATION_MASTER.md](../APPLICATION_MASTER.md), [../../AGENTS.md](../../AGENTS.md)
> - Phone: [../../../sync2dine-backend/docs/PHONE_ARCHITECTURE.md](../../../sync2dine-backend/docs/PHONE_ARCHITECTURE.md)
>
> This file preserves pre–Sync2Dine ops notes (`app.b-diddies.com`, `tradepro-*`, VPS `tradepro-api`) for archaeology only.
> Archived from APPLICATION_MASTER on 2026-07-23.
...
**Frontend tip commit:** `ccaed41` â€” CRM Add Lead + conversation notes/activities; platform_owner Cynthia CRM tools.

**Backend tip commit:** `6271370` â€” stale open-call expiry (45m) so Call Centre Live Status stops showing ghost cards.
...
**Ship note (2026-07-15 WhatsApp Path B):** Meta Cloud API disabled permanently (`WHATSAPP_META_ENABLED` off). Sole LIVE transport = WhatsApp Web.js QR (Â§18). Leave Meta secrets blank on VPS.

**Ship note (2026-07-15 Language boundary):** SPA UI English-only; phone/WhatsApp speech may use `SUPPORTED_LANGS`; brain + emails/contracts/quotes stay English (Â§3.6, Â§19.12). Vapi STT = Deepgram `multi`.

**Ship note (2026-07-16 Mid-call language + voices â€” AUDITED):** LIVE on `tradepro-api`. English TTS = **Lizzie locked** (`EQx6HGDYjkDpcli6vorJ`). Non-English = `server/phone-voices.ts` map (es Aerisita, pl Aleksandra, ru Klava, uk Kira, zh Zicai, fa Laura, sq Veronica). Identity always **Cynthia**. Tool `setCallLanguage` unchanged schema: persist preferred language + best-effort `PATCH /call/{id}` voice + speak-next nudge. Call-start prompt no longer hard-blocks non-English (â€œen-GB ONLYâ€ removed). No mid-call system-prompt rebuild; CRM tools unchanged. See Â§16.5 + [VOICE_SETUP.md](./VOICE_SETUP.md).

**Ship note (2026-07-16 Warm consult transfer â€” AUDITED):** LIVE. Cynthia mid-call handoff is **not** blind: Vapi `transferPlan.mode: warm-transfer-experimental` (hold caller â†’ dial staff â†’ brief â†’ bridge / cancel). Code: `tradepro-backend/server/transfer-numbers.ts` + `transferToHuman` in `vapi-routes.ts` / `phone-tools.ts`; prompts in `phone-brain.ts`. Destinations = Call Centre `/api/agent/transfer-numbers` (prod all â†’ `+447576442345`). Smoke: assistant destinations include warm plan Ã—5; live transfer with `transferPlan` â†’ `200 ok`. See Â§16.9 + [VOICE_SETUP.md](./VOICE_SETUP.md) + backend `docs/VAPI_SIP.md`.

**Ship note (2026-07-16 Lead management + Cynthia sales calls â€” AUDITED):** LIVE on SPA + `tradepro-api`. `/crm` Add Lead form works (was stub); lead detail **Conversation notes** timeline with aim + detail; callbacks strip with Call next / Mark done. `platform_owner` mapped in frontend agent context/rolePermissions like super_admin for CRM tools. Cynthia tools: `getLeadBrief`, `addLeadNote`, `listPendingCallbacks`; phone brain injects notes/activities into account memory. Outbound context carries `aim` + `customerId`; EOC writes `activities[]`. Outbound worker fixed â†’ `POST /api/calls/outbound` with `fromWorker:true` (no re-queue loop). `POST /api/leads/from-call` returns 4xx on capture errors. Live probe: platform owner created â€œAcme Trade SaaS Leadâ€ with notes; callbacks strip visible.

**Ship note (2026-07-16 Ghost active calls â€” AUDITED):** LIVE on `tradepro-api` `@ 6271370`. Call Centre **Live Call Status** is **not** live SIP presence â€” it lists open `calls` rows with `status` `ringing` | `in_progress` from `GET /api/agent/status` (`getAgentStatusSnapshot`). Missed Vapi end-of-call left multi-hour â€œOn callâ€¦â€ cards (debug: 4 ghosts, `elapsedSec` 46kâ€“446k). Fix: `expireStaleOpenCalls()` closes open rows older than **45 minutes** (`outcome: stale_timeout`, env `STALE_OPEN_CALL_MAX_MS`); also applied to lines `onCall`. Vapi hang/EOC/status-update expanded to always force `completed` + `endedAt`. Re-probe: `activeCalls: []`. See Â§16.7 / Â§16.10.

**Ship note (2026-07-17 Sync2Dine meal deals + dense delivery board â€” PRODUCTION):** LIVE on **https://app.sync2dine.io** (SPA `@ 189bb3e` / docs `@ 180bdf6`, bundle `index-BaNBGGFg.js`; API `@ cc06bdd`). Menu specials store `data.deal` (main/side/drink choices); Lizzie `placeFoodOrder` expands e.g. 3Ã— Mile a Meal â†’ ~9 kitchen lines; delivery board shows all lines compactly with deal hint. Demo: Huge Party Delivery tickets. See [MEAL_DEALS.md](./MEAL_DEALS.md).
...
| VPS backend git SHA | checkout tip may lag (`0ead751`); **live files** for language/voice = `d3e348f` content via SCP (`phone-voices.ts` present; `GET /api/vapi/health` â†’ 200) |
| WhatsApp Meta on VPS | `WHATSAPP_META_ENABLED=false` in `/etc/tradepro-api.env`; `/webhooks/whatsapp` â†’ 403 |
...
**Env key names on VPS** (values not recorded): `PORT`, `APP_BASE_URL`, `INTEGRATIONS_MOCK_MODE`, `JWT_SECRET`, `ORG_ENCRYPTION_KEY`, `SUPABASE_*`, `OPENAI_API_KEY`, `CURSOR_API_KEY`, `VOICE_PROVIDER`, `TELEPHONY_PROVIDER`, `VAPI_*`, `ELEVENLABS_*`, `SOHO66_*`, `VOICE_*`, `HOME_ORG_ID`, `DEFAULT_ORG_ID`, `GITHUB_TOKEN`, `WEBHOOK_BASE_URL`, â€¦ WhatsApp Meta keys (`WHATSAPP_META_ENABLED`, `WHATSAPP_ACCESS_TOKEN`, â€¦) must stay **off / blank** (Â§18).
...
| `/crm` | Lead Management CRM | super_admin, manager, staff, platform_owner | `ComprehensiveCRM` | LIVE â€” Add Lead, conversation notes/activities, callbacks strip |
...
| `/calls` | Call Centre (Cynthia phone) | staff+ | `CallCenter` | LIVE â€” Live Call Status = open `calls` rows (not SIP); stale auto-expire 45m |
...
| `mailbox/` | Mailbox service | PARTIAL | mock-default; live OAuth when secrets + mock off â€” Â§19.1 |
| `messaging/` | Hub, email/WhatsApp, PDFs, templates | PARTIAL | WA LIVE; Comms email often mock / SMTP-only â€” Â§19.1 |
...
| Layer | Language |
|-------|----------|
| **SPA UI** (menus, labels, screens) | **English only** â€” full UI i18n is not a ship goal; locale JSON trees under `src/app/i18n/locales/` are partial (AppShell chrome) / legacy |
| **Phone / WhatsApp / AI chat spoken replies** | Any `SUPPORTED_LANGS`: `en`, `sq`, `uk`, `ru`, `zh`, `es`, `pl`, `fa` |
| **Orchestrator / CRM / tool inputs** | Canonical **English** via `normalizeInboundText` â†’ `translateToEnglish` |
| **Customer artifacts** (emails, contracts, quotes, invoices, â€œsend this to themâ€) | **English only** via prompts + `ensureEnglishForCustomerSend` |
...
- Language packs panel = channel phrases + spoken-reply instructions (Â§19.12), not SPA strings.
- See Â§19.12 for APIs, local JSON storage, and phone `setCallLanguage`.
...
| Mailbox | `/api/mailbox/*`, Gmail/Outlook webhooks | PARTIAL | routes+poller LIVE; mock-default until secrets + mock off â€” Â§19.1 |
...
| Mailbox (schema only â€” unused by Node API) | `mailbox_connections`, `mailbox_tokens`, `mailbox_sync_state`, `email_messages_cache`, `email_attachments` â€” runtime SoT is `server/data/mailbox-data.json` (Â§19.1) |
...
| `email_oauth` | Messaging | PARTIAL | Code+poller LIVE; mock-default; CASA unverified â€” Â§19.1 |
...
5. Call Centre `/calls` â†’ live status (labeled Cynthia). Place real tests via Outbound Queue or inbound DID â€” not the Mock tab.
...
| ElevenLabs (live phone voice) | `ELEVENLABS_API_KEY`, `ELEVENLABS_VOICE_ID`, `ELEVENLABS_MODEL_ID` â€” prod Cockney Lizzie `EQx6HGDYjkDpcli6vorJ` (en locked) |
| Per-lang voice overrides (optional) | `VAPI_ELEVENLABS_VOICE_ID_ES` / `_PL` / `_RU` / `_UK` / `_ZH` / `_FA` / `_SQ`, or JSON `VAPI_ELEVENLABS_VOICE_MAP` â€” **must not override `en`** |
...
| Legacy (not live phone) | `CHATTERBOX_*` â€” mock/clone UI only; unused for Vapi media |
...
| Call start | `buildVapiAssistantForParty` â†’ `getVapiVoiceConfigForLang(language)` |
| Tool | `setCallLanguage` â€” same schema; persist + `call.metadata.callLanguage` / `callVoiceId` + best-effort Vapi voice PATCH |
| Prompt | Default Cockney English; may switch speech; **never** â€œen-GB ONLYâ€; always Cynthia |
| STT | Deepgram `language: multi` (unchanged) |
| Tools / CRM | Unchanged â€” tool args + customer artifacts stay English |
...
| Ops note | Library voices must exist on the ElevenLabs account wired to Vapi; missing â†’ Lizzie fallback + `voiceUpdated: false` log |
| Retest | Ask for Spanish/Polish mid-call â†’ must speak (not list-and-stop); back to English â†’ Lizzie. Full table: [VOICE_SETUP.md](./VOICE_SETUP.md) |

### 16.6 UI entry points

| Route / UI | Component | Notes |
|------------|-----------|-------|
| `/calls` | `CallCenter/CallCenter.tsx` | Call Centre â€” Cynthia; tabs: dashboard, lines, test, softphone, outbound (`?tab=`). **Live Call Status** polls `/api/agent/status` â†’ open `ringing`/`in_progress` rows only (not live SIP) |
...
| Chatterbox TTS | LEGACY for phone â€” not used for Vapi media; mock/clone UI only |
| Mock calls (`/api/agent/tts`, Test Call tab) | Intentional non-live â€” blocked in prod unless `ALLOW_TELEPHONY_MOCK` |
| Outbound worker URL | **LIVE** â€” worker dials `POST /api/calls/outbound` with `fromWorker:true` (fixed 2026-07-16; was `/api/phone/outbound`) |
| Ghost â€œOn callâ€ cards | **LIVE fixed 2026-07-16** â€” stale `ringing`/`in_progress` auto-expire at 45m (`expireStaleOpenCalls` / `stale_timeout`); was unbounded open rows |

### 16.8 Proven live stack (retest baseline)

Successful production calls used **Vapi + ElevenLabs Cockney (Lizzie `EQx6HGDYjkDpcli6vorJ`)**, not local STT/TTS. Retest: outbound to registered staff mobile + PIN, then inbound from a second phone to `SOHO66_FROM_NUMBER` / company DID. See [VOICE_SETUP.md](./VOICE_SETUP.md) Â§3.

### 16.9 Mid-call divert (Cynthia â†’ staff mobile) â€” LIVE (warm consult)
...
**Mode:** Vapi **`warm-transfer-experimental`** (not blind). Flow: tell caller they are on a short hold â†’ dial staff mobile â†’ Cynthia briefs staff â†’ `transferSuccessful` bridges caller, or `transferCancel` / `fallbackPlan` returns to Cynthia. Built in `server/transfer-numbers.ts` (`buildWarmTransferPlan` / `resolveTransferDestination`).

| Piece | Detail |
|-------|--------|
| UI | `/calls` â†’ Phone Lines â†’ **Call Transfer Destinations** (`general` / `sales` / `projects` / `recruitment` / `accounts`) |
| API | `GET` / `PATCH` `/api/agent/transfer-numbers` â†’ `agentSettings.transferNumbers` |
| Prod dest | All departments â†’ `+447576442345` (admin mobile) |
...
| Inbound DID | Soho66 **Ring my IP phone** â†’ VPS Asterisk REGISTER bridge (`docker/soho66-vapi-bridge`) â†’ Vapi/Lizzie â€” no SIP-URL forward, no in-app softphone required |
| Constraint | Only **one** REGISTER on SIP user `1005090093` â€” keep VOIS logged out while the bridge owns inbound |
...
| Smoke (2026-07-16) | Assistant destinations = `warm-transfer-experimental` Ã—5; live control transfer with full `transferPlan` â†’ `200 ok` (`endedReason: call.in-progress.sip-completed-call`). |

### 16.10 Live Call Status (how it works)

| Piece | Detail |
|-------|--------|
| UI | `/calls` dashboard amber cards (â€œOn call withâ€¦â€) |
| API | `GET /api/agent/status` â†’ `activeCalls` / `activeCall` |
| Source of truth | Persisted `calls[]` in `data-store` with `status` âˆˆ `{ringing, in_progress}` |
| Elapsed | `computeCallDurationSec` = now âˆ’ `startedAt` when no `endedAt` (inflates if webhook missed) |
| Stale TTL | `STALE_OPEN_CALL_MAX_MS` default **45 minutes** â€” closed with `outcome: stale_timeout` on status poll |
| Today stats | Separate filter (started today + completed) â€” can show â€œ1 call todayâ€ while old ghosts previously still appeared as active |
...
| `setCallLanguage` | no | no | read â€” saves `callLanguage` + CRM/profile preferred language; best-effort Vapi voice PATCH from `phone-voices.ts` (en=Lizzie locked); STT Deepgram `multi` (Â§19.12) |
...
**Status:** LIVE on VPS via **WhatsApp Web.js only** (QR API 200; backend WWeb client). Path B Meta Cloud API is **DISABLED permanently** â€” source retained cold; **do not enable** in production.
...
- Staff chat = long-lived real numbers via QR session only (`server/data/.wwebjs_auth/` â€” DO_NOT_SHIP)

### 18.1 Path A â€” WhatsApp Web.js (sole LIVE transport)
...
| Hub send | `POST /api/messages/send` â€” WWeb when `ready`; else 400 (connect QR) |
...
### 18.2 Path B â€” Meta Cloud API (DISABLED / cold forever)

**Status:** DISABLED permanently. Code kept for archaeology only â€” not a product path. Requires `WHATSAPP_META_ENABLED=true` (never set in production).

| Piece | Detail |
|-------|--------|
| Handler | `server/whatsapp-webhook.ts` (both repos; prod = backend) â€” Graph choke + inert webhook when flag off |
| Webhook | `GET/POST /webhooks/whatsapp` â€” GET 403 / POST empty ack when disabled |
...
| `whatsappProvider` Meta-token false mock | FIXED â€” mock only on mock-mode / disabled; backend chooses WWeb |
| Concierge WhatsApp outbound | Returns note only â€” PARTIAL |
| Project WhatsApp groups | Unavailable (Meta Groups cold by design) â€” use 1:1 + portal |
...
| | |
|---|---|
| **Status** | PARTIAL â€” LIVE code+poller; mock-default until secrets + mock off; CASA production verification open â€” see [CASA_MAILBOX_CHECKLIST.md](./CASA_MAILBOX_CHECKLIST.md) |
| **UI** | Settings â†’ Email & Inbox; `/communications` (Mailbox / Inbox / Compose); Integrations â†’ `email_oauth` / `email_smtp` |
| **API** | `/api/mailbox/connect`, `/callback`, `/connections`, `/sync`, `/messages`, `/send`, `/search`; `POST /webhooks/gmail`, `/webhooks/outlook` |
| **Env** | `TOKEN_ENCRYPTION_KEY` (required in prod â€” weak hardcoded fallback if unset), `MAILBOX_OAUTH_REDIRECT_BASE` (prod: `https://app.b-diddies.com`), `GOOGLE_OAUTH_*`, `MICROSOFT_OAUTH_*` / `MICROSOFT_OAUTH_TENANT_ID`, `YAHOO_OAUTH_*`, `INTEGRATIONS_MOCK_MODE` / `MAILBOX_MOCK_MODE` / `MAILBOX_FORCE_MOCK`, optional `AUTH_ENFORCED` |
| **Worker** | `startMailboxPoller()` on API listen â€” tick every **60s**; per-connection due uses `pollIntervalSec` (default **180**). There is **no** `MAILBOX_POLL_INTERVAL_MS` reader. |
| **Data** | Runtime SoT: `server/data/mailbox-data.json` on the API host. Supabase mailbox tables exist as schema only â€” Node store does not read them yet. |
...
| **Google scope** | `https://mail.google.com/` (full mail â€” restricted; drives CASA). |
| **Stacks** | (1) Mailbox OAuth IMAP sync + XOAUTH2 SMTP send via `/api/mailbox/*`. (2) Comms hub transactional email via `/api/messages/send` â†’ SMTP only (`email_smtp`). Do not confuse the two. |
...
| **Google Cloud setup** | Create OAuth client type **Web application**. Authorized JavaScript origins: `https://app.b-diddies.com`. Authorized redirect URIs: `https://app.b-diddies.com/api/mailbox/callback`. Paste Client ID/Secret in Integrations â†’ Mailbox OAuth (in-app setup guide on that card). |
...
| **Store** | **Primary:** Supabase `code_fix_jobs`. Local `server/data/code-fix-jobs.json` is cache/fallback only. Auto-start surgical fixes defaults **off** (AI Studio toggle). |
...
|---|---|
| **Status** | LIVE |
| **Product rule** | Â§3.6 â€” spoken/chat may be non-English; SPA UI + customer artifacts stay English |
| **UI** | AI Studio â†’ Language packs (`LanguagePacksPanel`); Profile/Team preferred language = chat/phone only |
...
| **Channel flow** | Inbound â†’ `normalizeInboundText` (English for brain); conversational outbound â†’ `localizeOutboundText`; emails/contracts/quotes â†’ English guard |
...
| **Note** | Frontend companion `server/data/` twin is not production â€” save packs via API on VPS / keep backend file in sync |
...
| Mailbox OAuth / IMAP | YES | PARTIAL | Â§19.1 â€” mock-default; CASA open |
| Legacy EmailSystem | YES (orphan) | ORPHAN | Â§28.14 |
| WhatsApp Web (Meta Path B cold forever) | YES | DONE | Â§18 |
...
| English UI + multilingual speech boundary | YES | DONE | Â§3.6, Â§19.12 |
...
| i18n locales (partial AppShell only; UI English-only policy) | YES | PARTIAL | Â§3.6, Â§27 locale bridge |
...
| `LEGACY FE/server` | Do not treat as SoT â€” edit `BE/server` only |
...
| Meta WA (cold) | `/webhooks/whatsapp` inert | â€” | â€” | `whatsapp-webhook.ts` gated by `WHATSAPP_META_ENABLED` | â€” |
